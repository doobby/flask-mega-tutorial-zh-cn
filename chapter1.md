...
首先需要安装 Python 解释器。如果你的操作系统没有提供安装包，可以去[官网](http://python.org/download/)下载。注意如果你用的是 Windows 的 WSL 或者 Cygwin 环境，应该选择 Ubuntu (WSL) 或者 Cygwin 的版本，而非 Windows 原生 Python。

要验证 Python 是否正常工作，可以打开一个 terminal 窗口，输入 `python3` 或者 `python`。正常的话应该会看到解释器运行
...
此时 Python 解释器在等待输入。在后面的章节里，你将学习到交互式命令行的各种功能。不过现在仅仅是确认 Python 被安装好了。输入 `exit()` 加回车退出 Python 命令行。也可以在 Linux 或者 Mac OS X 上使用快捷键 `Ctrl-D`，Windows 上使用 `Ctrl-Z` 加回车退出。

## 安装 Flask

接下来安装 `Flask`，但在此之前先来说明一下 Python 包安装的最佳方法。

Python 的包（比如 Flask）被保存在一个公共仓库，任何人都可以下载并且安装它们。Python 官方的包仓库被称为 [PyPI](https://pypi.python.org/pypi) (Python Package Index，也有人把这个仓库称作 cheese shop)。

要从 PyPI 上获取并安装软件包非常容易，Python 提供了一个 `pip` 工具来完成这件工件（在 Python 27 中 `pip` 尚未被打包在标准安装包中，需要单独来安装它）。

下面的命令演示如何使用 pip 安装一个包

```bash
$ pip install <package-name>
```

大多数情况下该命令不会成功。因为当 Python 解释器被全局安装（可以被计算机上的所有用户访问），作为普通用户你是没有权限来对 Python 进行修改的，只有管理员用户才能正常安装。`pip` 工具从 PyPI 上下载包，当默认把它安装到 Python 的标准目录中，这样所有的用户都可以使用这个包。
...
要解决这一问题——为不同的应用配置不同版本的包，Python 提供了 Virutal Environment （虚拟环境）的概念。Virtual Environment 是一份完整的 Python 解释器副本。当你在 Virtual Environment 中安装包时，系统级的 Python 并不受影响，只有当前虚拟环境的副本会被修改。你可以自由为每个应用配置不同的 Python 虚拟环境。Virtual Environment 由创建者所有，因此不需要他们具有管理员权限。
...
上述命令指示 Python 运行 venv 包，告诉它来创建一个名为 `venv` 的 Virtual Environment。第一个 `venv` 是 Python Virutal Environment 包的名称，第二个是要创建的 Virtual Environment 包的名称。如果你对此仍有困惑，那么你可以他第二个 `venv` 改成任何名称来表示 Virtual Environment。一般习惯会在工程目录下创建一个 `venv` 目录来作为  Virtual Environment 环境，这样切到工程目录时就能找到对应的 Virtual Environment 环境。
...
激活的 virtual environment 将作用于当前 terminal 会话。命令提示符也发生了变化提醒你在虚拟环境中。Virtual Environment 仅临时并且私有的在当前会话内生效，关闭 terminal 将自动退出。甚至在你同时打开了多个 terminal 时，可以为每个 terminal 配置使用不同的虚拟环境。
...
上面的脚本简单的创建了一个 Flask 类的实例，Flask 类由 flask 包中导入。传给 `Flask` 类的变量 `__name__` 是 Python 预定义的变量，表示当前模块的名称。Flask 根据这个模块的位置来确定相应的资源的目录，比如在[第二章](chapter2.md)中将会提到的 template （模块）文件。一般来说，使用 `__name__` 能适应大部分应用场景。最后我们导入了 `routes` 模块，但还没有使用到。
...
装饰器是 Python 语言的一种特殊语法，用来修改其装饰的函数的功能。装饰器常用于将函数注册为某种事件的回调。在这个例子中，装饰器建立了 URL 和回调函数的连接关系，URL '/' 和 '/index' 都被关联到这一函数。这表示当浏览器访问这两个 URL 时，Flask 将调用这个函数，并将返回值作为回复给浏览器。当然目前返回值过于简单，意义还不那么大。

我们需要再新建一个 Python 脚本，使用我们定义的包。创建名为 `microblog.py` 的脚本（主入口），其中只有一行，导入了 Flask 实例

```python
from app import app
```

还记得我们之前区分两个 `app` 的说明吗？这里你在同一句中看到了两个 `app`。后者表示 Flask 应用实例，前者表示由 `app` 包中导出。如果你还是觉得迷惑，不妨把包名或者应用实例名改成其它名字。
...
按 Ctrl-C 结束服务，现在我们完成 web 开发者的第一步，意义重大的一步。
